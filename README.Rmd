...
The set of known publications is maintained as a lookup table inside the package. Since the WEO database moved to the [IMF Data portal](https://data.imf.org) with the October 2025 release, each vintage sits behind an opaque per-vintage identifier that cannot be derived from the year and release, so the latest publication can no longer be detected online. A new release becomes available after a package update.
