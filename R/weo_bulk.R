...
create_weo_url <- function(year, release) {
  ...
  if (year >= 2024) {
    sprintf("%s/%d/%s/WEO%s%dall.xls",
            base_url, year, month_long, month, year)
  } else if (year >= 2021) {
    # Format from April 2021 to 2023
    sprintf("%s/%d/WEO%s%dall.ashx",
            base_url, year, month, year)
  } else if (year >= 2020) {
    # Format from October 2020
    sprintf("%s/%d/%02d/WEO%s%dall.xls",
            base_url, year, release, month, year)
  } else {
    # Earlier format
    sprintf("%s/%d/WEO%s%dall.xls",
            base_url, year, month, year)
  }
}
...
process_weo_data <- function(raw_data) {
  required_cols <- c("Country", "ISO", "Subject Descriptor",
                     "Units", "WEO Subject Code")
